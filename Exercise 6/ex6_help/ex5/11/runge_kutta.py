...
def lorenz_model(x,t,params):
    ...
    f=0.0*x
    f[0]=params[0]*(x[1]-x[0])
    f[1]=params[1]*x[0]-x[1]-x[0]*x[2]
...
def main():
    ...
    t, sol_ode = odeint_test(ax2)
    ax2.set_title('odeint')
    tr, sol_r = runge_kutta_test(ax3)
    ax3.set_title('own Runge-Kutta 4')
    
    print('maximum difference: ', np.amax(abs(sol_ode-sol_r)))
    show()
