...
Originally by Ilkka Kylanpaa on January 2019,
"""
...
# Define the basis functions for linear interpolation.
def l1(t):
    return 1 - t
...
class linear_interp:
    """
    A class that can be used to linearly interpolate a 1D, 2D and 3D functions.
    Takes the coordinate points as numpy.linspace or numpy.meshgrid and the function,
    ...
    def eval1d(self,x):
        # 1D evaluation function. Takes an array or a scalar value for x and linearly approximates
        # the function using the values near the points. Returns the values in an array.
        if np.isscalar(x):
            x=np.array([x])
        N=len(self.x)-1
        f=np.zeros((len(x),))
    ...
    def eval2d(self,x,y):
        # 2D evaluation function. Takes two arrays or a scalar values for x and y and linearly approximates
        # the function using the values near the points. Returns the values in an array.
        if np.isscalar(x):
            x=np.array([x])
        if np.isscalar(y):
    ...
    def eval3d(self,x,y,z):
        # 3D evaluation function. Takes three arrays or a scalar values for x, y and z and linearly approximates
        # the function using the values near the points. Returns the values in an array.
        if np.isscalar(x):
            x=np.array([x])
        if np.isscalar(y):
...
def main():
    """
    Function that makes a few example graphs and runs tests.
    """
    fig1d = plt.figure()
    ...
    test_interpolation = linear_interp(x=x,y=y,z=z,f=F,dims=3)
    value = test_interpolation.eval3d(2, 2, 2)
    if abs(value - 12) > 0.01:
        print("linear interpolation is not working!")
    else:
