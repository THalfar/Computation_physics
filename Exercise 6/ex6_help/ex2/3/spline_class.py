...
"""
The basis functions for qubic germite splines
"""
def p1(t):
    return (1+2*t)*(t-1)**2
...
class spline:
    """
    A class for interpolating 1D, 2D or 3D functions using cubic hermite splines.
    Takes arrays of coordinates
    """
...
def main():
    ...
    test_interpolation = spline(x=x,y=y,z=z,f=F,dims=3)
    value = test_interpolation.eval3d(2, 2, 2)
    if abs(value - 12) > 0.01:
        print("spline is not working!")
    else:
