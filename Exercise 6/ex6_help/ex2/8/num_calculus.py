...
also exercise 2 - problem 3 - n dimensional gradient function
with simple testing num_gradient
...
def num_gradient( function, x, dx):
    ...
    # returns the gradient of n-dimensional function
    # assume same grid size in every direction ie dx = dx for all
    # accuracy is of order dx
...
def test_first_derivative():
    ...
    print("difference is {} %".format(abs(value-correct)*100/correct))
    print()
...
def test_num_gradient():
    ...
    def fun1(x): return float(3*x[0]**2+x[1])
    
    value = num_gradient(fun1, np.array([2.0,2.0]), 0.001)
    correct = [12,1]
    
    if abs(value[0] - correct[0])/correct[0] < 0.001 and \
        abs(value[1] - correct[1])/correct[1] < 0.001:
        print("3rd (nth) derivative correct for simple polynomials")
        
    else:
        print("3rd (nth) derivative incorrect for simple polynomials")
        failures += 1
    ...
    def fun2(x): return float(np.sin(x[0]*x[1])*x[2])
    
    value = num_gradient(fun2, np.array([2.0,2.0,2.0]), 0.001)
    correct = [4*np.cos(4), 4*np.cos(4), np.sin(4)]
    
    if abs(value[0] - correct[0])/correct[0] < 0.001 and \
        abs(value[1] - correct[1])/correct[1] < 0.001and \
        abs(value[2] - correct[2])/correct[2] < 0.001:
        print("7th (nth) derivative correct for a sine function")
        
    else:
        print("7th (nth) incorrect for a sine function")
        failures += 1
        
    print("correct value is <{}>, \
        functions value is <{}>".format(correct,value))
    print("difference is {} %".format(abs(value-correct)*100/correct))
    print()
    print("function checked was sin(x)")
    print()
