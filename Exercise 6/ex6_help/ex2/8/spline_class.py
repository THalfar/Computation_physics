...
def test_1d(f,x):
    ...
    spl = spline(x=x, f=f(x), dims = 1)
    dif = sqrt((spl.eval1d(x_test)-f(x_test))**2)
    print(dif)
...
def test_2d(f,x,y):
    ...
    spl = spline(x=x, y=y, f=f(x,y), dims = 2)
    dif = sqrt((spl.eval2d(x_test,y_test)-f(x_test,y_test))**2)
    print(dif)
...
def test_3d(f,x,y,z):
    ...
    spl = spline(x=x, y=y, z=z, f=f(x,y,z), dims = 3)
    dif = sqrt((spl.eval3d(x_test,y_test,z_test)-f(x_test,y_test,z_test))**2)
    print(dif)
...
def main():
    ...
    test_1d(sin, linspace(0,pi,20))
    
    def fun2(x,y): 
        # nested function to test 2d interpolation to
        
        result = zeros((x.size,y.size))
        
        for i in range(1,x.size):
            for j in range(1,y.size):
                result[i,j] = sin(x[i]*y[j])
                
        
        return result
        
    test_2d(fun2, linspace(0,pi,20),linspace(0,pi,20))
    
    def fun3(x,y,z): 
        # nested function to test 3d interpolation to
        
        result = zeros((x.size,y.size,z.size))
        
        for i in range(1,x.size):
            for j in range(1,y.size):
                    for k in range(1,z.size):
                        result[i,j,k] = sin(x[i]*y[j]*z[k])
                
        
        return result
        
    test_3d(fun3, linspace(0,pi,20),linspace(0,pi,20),linspace(0,pi,20))
#end main
