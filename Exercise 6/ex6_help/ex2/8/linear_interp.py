...
def l1(t): 
    """
    :param: t  is the location in the grid square [0,1]
    
    :return:  location from the further side
    """
...
def l2(t):
    ...
    :return:  location from the closer side
    """
...
class linear_interp:
    ...
    def __init__(self,*args,**kwargs):
        """
        :param: dims   num of dimensions
        :param: x      x-coordinates of grid
        :param: y      y-coordinates of grid
        :param: z      z-coordinates of grid
        :param: f      function
        """
        
        # assigns gridpoints, function and gridseparations
        self.dims=kwargs['dims']
            # assigns 1d case of the interpolation
        if (self.dims==1):
            self.x=kwargs['x']
            self.f=kwargs['f']
            self.hx=np.diff(self.x)
        elif (self.dims==2):
            # assigns 2d case of the interpolation
            self.x=kwargs['x']
            self.y=kwargs['y']
            self.f=kwargs['f']
            self.hx=np.diff(self.x)
            ...
            # assigns 3d case of the interpolation
            self.x=kwargs['x']
            self.y=kwargs['y']
            self.z=kwargs['z']
            self.f=kwargs['f']
    ...
    def eval1d(self,x):
        # evaluates value of the 1d function interpolation
    ...
    def eval2d(self,x,y):
        # evaluates value of the 2d function interpolation
    ...
    def eval3d(self,x,y,z):
        # evaluates value of the 3d function interpolation
...
def test_1d(f,x):
    # tests function value to interpolation value at little off center of
    # the grid
    x_test = x[int(round(x.size/2))] + 0.7*(x[1]-x[0])
    
    lin = linear_interp(x=x, f=f(x), dims = 1)
    dif = np.sqrt((lin.eval1d(x_test)-f(x_test))**2)
    print(dif)
...
def test_2d(f,x,y):
    ...
    # the grid
    x_test = x[int(round(x.size/2))] + 0.7*(x[1]-x[0])
    y_test = y[int(round(y.size/2))] + 0.7*(y[1]-y[0])
    
    
    
    lin = linear_interp(x=x, y=y, f=f(x,y), dims = 2)
    dif = np.sqrt((lin.eval2d(x_test,y_test)-f(x_test,y_test))**2)
    print(dif)
...
def test_3d(f,x,y,z):
    ...
    x_test = x[int(round(x.size/2))] + 0.7*(x[1]-x[0])
    y_test = y[int(round(y.size/2))] + 0.7*(y[1]-y[0])
    z_test = z[int(round(z.size/2))] + 0.7*(z[1]-z[0])
    
    
    lin = linear_interp(x=x, y=y, z=z, f=f(x,y,z), dims = 3)
    dif = np.sqrt((lin.eval3d(x_test,y_test,z_test)-f(x_test,y_test,z_test))**2)
    print(dif)
...
def main():
    ...
    test_1d(np.sin, np.linspace(0,np.pi,20))
    def fun(x,y): 
        # nested function to test 2d interpolation to
        
        result = np.zeros((x.size,y.size))
        
        for i in range(1,x.size):
            for j in range(1,y.size):
                result[i,j] = np.sin(x[i]*y[j])
                
        
        return result
        
    test_2d(fun, np.linspace(0,np.pi,20),np.linspace(0,np.pi,20))
    
    def fun3(x,y,z): 
        # nested function to test 3d interpolation to
        ...
        for i in range(1,x.size):
            for j in range(1,y.size):
                    for k in range(1,z.size):
                        result[i,j,k] = np.sin(x[i]*y[j]*z[k])
                
        
        return result
        
    test_3d(fun3, np.linspace(0,np.pi,20),np.linspace(0,np.pi,20),np.linspace(0,np.pi,20))
#end main
