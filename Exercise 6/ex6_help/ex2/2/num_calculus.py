...
def gradient(function, x, dx):
    """ 
    This calculates the N-dimensional numerical gradient
    """
    N = len(x)
    G = []
    for dim in range(N):
        ...
        # Construct points x + dx and x - dx for a specified dimension
        x_dx1[dim] += dx[dim]
        x_dx2[dim] -= dx[dim]

        # result is a "vector" containing derivatives
        G.append((function(x_dx1) - function(x_dx2)) /2. /dx[dim])
    return G

def gradient_descent(function, x, dx):
    """
    Calculates the local minimum of a function.
    function = function to be evaluated
    x = iteration starting value
    dx = size of differential for gradient
    """
    a = 1e-3
    threshold = 1e-3
    x_iter = x.copy()
    dx = np.ones(3) * threshold # test value same for all dimensions, can be configured
    grad_norm = np.linalg.norm(gradient(function, x_iter, dx)) # |g'(x_N)| in 31

    # iterate while gradient is larger that the threshold: stopping means a local minimum
    while grad_norm > threshold:
        gamma = a / (grad_norm + 1) # eq. 31
        grad_norm = np.linalg.norm(gradient(function, x_iter, dx))
        # move iter "down the gradient" by amount specified by a
        x_iter = np.subtract(x_iter, gamma * np.array(gradient(function, x_iter, dx))) # eq. 32
    return x_iter
...
def test_gradient(tolerance=1.0e-3):
    """ Test routine for gradient of f"""
    x = [1, -2]
    dx = [0.01, 0.01]
    estimate = gradient(test_fun_multivariate, x, dx)
    exact = [-16, 12] # point from https://ch.mathworks.com/help/matlab/ref/gradient.html
    err = sum(np.abs(np.array(estimate) - np.array(exact)))
    working = False
    if (err < tolerance):
...
def test_gradient_descent(tolerance=1.0e-3):
    """ Test routine for gradient descent of f"""
    x = [1, -2]
    dx = [0.01, 0.01]
    estimate = gradient_descent(test_fun_local_minimum, x, dx)
    exact = [1.5, -1.5] # wolfram alpha
    err = sum(np.abs(np.array(estimate) - np.array(exact)))
    working = False
    if (err < tolerance):
...
def test_fun_multivariate(x):
    """ This is the test function used in unit testing"""
    return x[0]**2 * x[1]**3

def test_fun_local_minimum(x):
    """ This is the test function used in unit testing"""
    # from https://www.wolframalpha.com/widgets/view.jsp?id=c3dca6eecdf620eb762db0179b82f5
    return x[0]**3 - 3*x[0]*x[1]**2 + x[1]**4
